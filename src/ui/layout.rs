...
fn compute_cursor_screen_pos(state: &AppState, request_area: Rect, body_area: Rect) -> (u16, u16) {
    ...
    match state.active_panel {
        Panel::Body => {
            let cursor_row_on_screen = (state.body_vim.cursor_row as u16).saturating_sub(state.body_vim.scroll_offset as u16);
            let cursor_col_on_screen = (state.body_vim.cursor_col as u16).saturating_sub(state.body_hscroll as u16);
