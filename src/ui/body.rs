...
pub fn render(frame: &mut Frame, state: &AppState, area: Rect) {
    ...
    let scroll_y = state.body_vim.scroll_offset;
    let hscroll = state.body_hscroll;
    let visible_height = inner.height as usize;
    let cursor_row = state.body_vim.cursor_row;

    // Compute bracket match
    let matched_bracket = if is_focused {
        find_matching_bracket(&body_lines, state.body_vim.cursor_row, state.body_vim.cursor_col)
    } else {
        None
    };
...
fn visual_range(state: &AppState) -> (usize, usize, usize, usize) {
    let (ar, ac) = state.body_vim.visual_anchor.unwrap_or((0, 0));
    let (cr, cc) = (state.body_vim.cursor_row, state.body_vim.cursor_col);
    if (ar, ac) <= (cr, cc) {
        (ar, ac, cr, cc)
    } else {
        (cr, cc, ar, ac)
...
fn highlight_body_search_line(
    ...
    if query_lower.is_empty() {
        if is_cursor_line {
            return render_normal_cursor_line(line, state.body_vim.cursor_col.saturating_sub(hscroll), t);
        }
