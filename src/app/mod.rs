...
impl App {
    ...
    fn sync_type_vim_text(&mut self) {
        self.state.response_type_text = self.state.type_vim.content();
    }
    ...
    fn inline_input(&mut self, c: char) {
        match self.state.active_panel {
            ...
            Panel::Response if self.state.response_tab == ResponseTab::Type && self.state.type_sub_focus == crate::state::TypeSubFocus::Editor => {
                ...
                let pos = row_col_to_offset(text, self.state.type_vim.cursor_row, self.state.type_vim.cursor_col);
                text.insert(pos, c);
                self.state.type_vim.cursor_col += 1;
                self.state.response_type_locked = true;
            }
                ...
                if pos > 0 {
                    ...
                    if ch == b'\n' {
                        if self.state.type_vim.cursor_row > 0 {
                            ...
                            let lines: Vec<&str> = text.lines().collect();
                            self.state.type_vim.cursor_col = lines.get(self.state.type_vim.cursor_row).map(|l| l.len()).unwrap_or(0);
                        }
                    } else {
                        self.state.type_vim.cursor_col = self.state.type_vim.cursor_col.saturating_sub(1);
                    }
                    self.state.response_type_locked = true;
                }
            }
    ...
    fn inline_newline(&mut self) {
        if self.state.active_panel == Panel::Response && self.state.response_tab == ResponseTab::Type && self.state.type_sub_focus == crate::state::TypeSubFocus::Editor {
            ...
            let lines: Vec<&str> = text.lines().collect();
            let current_line = lines.get(self.state.type_vim.cursor_row).copied().unwrap_or("");
            ...
            self.state.type_vim.cursor_row += 1;
            self.state.type_vim.cursor_col = leading_ws.len() + extra_indent.len();
            self.state.response_type_locked = true;
            return;
        }
        if self.state.active_panel == Panel::Body {
    ...
    fn inline_cursor_left(&mut self) {
        match self.state.active_panel {
            ...
            Panel::Response if self.state.response_tab == ResponseTab::Type && self.state.type_sub_focus == crate::state::TypeSubFocus::Editor => {
                if self.state.type_vim.cursor_col > 0 {
                    self.state.type_vim.cursor_col -= 1;
                } else if self.state.mode == InputMode::Insert && self.state.type_vim.cursor_row > 0 {
            ...
            Panel::Response => {
                self.state.resp_vim.cursor_col = self.state.resp_vim.cursor_col.saturating_sub(1);
            }
            _ => {}
        }
    ...
    fn inline_cursor_right(&mut self) {
        ...
        match self.state.active_panel {
            ...
            Panel::Response if self.state.response_tab == ResponseTab::Type && self.state.type_sub_focus == crate::state::TypeSubFocus::Editor => {
                ...
                if self.state.type_vim.cursor_col < max {
                    self.state.type_vim.cursor_col += 1;
                } else if is_insert && self.state.type_vim.cursor_row + 1 < lines.len() {
                    self.state.type_vim.cursor_row += 1;
                    self.state.type_vim.cursor_col = 0;
                }
                // type_vim hscroll handled by ensure_cursor_visible
            ...
            Panel::Response => {
                ...
                if self.state.resp_vim.cursor_col < max {
                    self.state.resp_vim.cursor_col += 1;
                }
            }
            _ => {}
        }
    ...
    fn inline_cursor_home(&mut self) {
        match self.state.active_panel {
            ...
            Panel::Response => { self.state.resp_vim.cursor_col = 0; self.sync_resp_hscroll(); },
            Panel::Request => match self.state.request_focus {
    ...
    fn inline_cursor_end(&mut self) {
        ...
        match self.state.active_panel {
            ...
            Panel::Response if self.state.response_tab == ResponseTab::Type && self.state.type_sub_focus == crate::state::TypeSubFocus::Editor && is_insert => {
                let lines: Vec<&str> = self.state.response_type_text.lines().collect();
                let line_len = lines.get(self.state.type_vim.cursor_row).map(|l| l.len()).unwrap_or(0);
                self.state.type_vim.cursor_col = line_len;
            }
            Panel::Response => {
                let lines = self.get_response_lines();
                let line_len = lines.get(self.state.resp_vim.cursor_row).map(|l| l.len()).unwrap_or(0);
                self.state.resp_vim.cursor_col = line_len.saturating_sub(1);
                self.sync_resp_hscroll();
            }
            Panel::Request => {
                let len = self.get_request_field_len();
    ...
    fn body_cursor_ptrs(&mut self) -> (String, *mut usize, *mut usize) {
        match self.state.active_panel {
            Panel::Response => {
                ...
                (t, &mut self.state.resp_vim.cursor_row as *mut usize, &mut self.state.resp_vim.cursor_col as *mut usize)
            }
            _ => {
